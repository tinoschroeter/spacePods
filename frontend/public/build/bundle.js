(() => {
  ...
  var _Pod = class {
    ...
    destroy() {
      fetch(`${_Pod.api}/${this.podName}`, {
        method: "DELETE"
      }).then((res) => res.json()).then((data) => console.log(data)).catch((e) => console.error(e));
      this.remove();
    }
    destroyShip(ship) {
  ...
  __publicField(Pod, "api", "http://localhost:3000/api/pods");
  ...
  var SpaceShip = class {
    ...
    showPods() {
      const show = () => this.podDisplay.innerText = Pod.count ? `Pods:  ${Pod.count}` : `Pods:  0`;
      show();
